...
class Mydumper(BackupBase):
    ...
    def __init__(self, name, dir, backup_bin_file='mysqldump'):
        ...
        """
        super(Mydumper, self).__init__(name, dir)
    ...
    def backup_data(self, username='', password='', host='127.0.0.1',
        ...
        cmd = (
            ...
            ' {param} > {dir}/{name}'.format(
                                     ...
                                     port = port,
                                     param = param,
                                     dir = super(Mydumper, self).dir,
                                     name = super(Mydumper, self).name)
        )
        is_ok = super(Mydumper, self).backup_data(cmd)
    ...
    def valid(self, host = '127.0.0.1', username = '', password = ''):
        ...
        # 如果备份没有传输到远程则无法进行校验
        if not super(Mydumper, self).remote_backup_file:
            msg = '没有远程备份集'
            return is_ok, msg

        # 构造返回信息
        if super(Mydumper, self).compress_file:
            msg = '压缩备份 ({status})'
        elif super(Mydumper, self).backup_file:
            msg = '未压缩备份 ({status})'
        else:
            msg = '没有备份集'
            return is_ok, msg
        ...
        if (super(Mydumper, self).checksum() ==
            super(Mydumper, self).checksum_remote(host = host,
                                            username = username,
                                            password = password)):
            is_ok = True
...
def main():
    ...
    mydumper = Mydumper('mydumper_2.sql', '/tmp')

    print mydumper.backup_data(param = param, **db_conf)
    print mydumper.checksum()
    print mydumper.send_backup(remote_file = '/u01/backup/mydumper_2.sql',
                                **os_conf)

    print mydumper.checksum_remote(**os_conf)
    print mydumper.backup_size()
    print mydumper.compress(compress_type='gzip', file_type='file')
    print mydumper.checksum()
    print mydumper.backup_size()
    print mydumper.send_backup(
                    remote_file = '/u01/backup/mydumper_2.sql.tar.gz',
                    **os_conf)

    print mydumper.valid(**os_conf)
    print mydumper.checksum_remote(**os_conf)
    print mydumper.backup_binlog(from_dir = '/root',
                                  to_dir = '/u01/backup/binlog',
                                  cmin = 86400)

    print mydumper.send_binlog(remote_dir = '/u02/backup/binlog',
                                **os_conf)
